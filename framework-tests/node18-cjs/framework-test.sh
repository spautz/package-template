...
CONTAINER_ID=$(docker ps -a --filter=name=node18-cjs-main-container --format "{{.ID}}" --last 1)
...
if [[ $EXIT_CODE -eq 0 ]]; then
  run_command docker cp $CONTAINER_ID:/framework-test-node18-cjs/package-lock.json ./
else
  exit $EXIT_CODE
fi
