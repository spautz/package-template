# External-Test: Node 26 with pnpm (no Corepack), resolving ESM
...
To run locally:
```bash
scripts/run-external-test.sh  node26-pnpm-esm
```

or, to get a shell and play around on your own:
```bash
scripts/run-external-test-interactive.sh  node26-pnpm-esm
```
